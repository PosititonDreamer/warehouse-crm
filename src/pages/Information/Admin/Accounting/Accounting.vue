<script>
    ...
    export default {
        ...
        setup() {
            ...
            const { getAccounting, findAccounting, newAccounting, updateAccounting, deleteAccounting, updateAmountAccounting } = accounting()
...
<template>
    <div class="accounting">
        ...
        <div class="accounting__header">
            <u-button v-for="warehouse in getWarehouses" :class="[{'u-button--active': getPage === warehouse.id}]" @click="changePage(warehouse.id)">{{warehouse.title}}</u-button>
        </div>
        <div class="accounting__content" v-if="getActiveWarehouse">
            ...
            <form @submit.prevent="saveAccounting" class="accounting__form" @reset="closeModal" v-if="openModal">
                <div class="accounting__content-form" v-if="!getLoading">
                    ...
                    <U-Input :error="errorProduct" @change="changeProduct" class="accounting__input" name="Выберите продукт" view="select" v-model="product" :start-value="getProduct" key="product-name" :disabled="pageModal === 'editAmount'">
                        <option value=""></option>
                        <option v-for="product in getProducts" :value="product.id">{{ product.title }}</option>
                    </U-Input>
                    <U-Input :error="errorCount" @change="takedCount = true" class="accounting__input" name="Введите количество в упаковке" type="number" v-model="count" :start-value="getCount.toString()" key="product-count" :disabled="pageModal === 'editAmount'"/>
                    <U-Input :error="errorMeasureUnit" @change="takedMeasureUnit = true" class="accounting__input" name="Введите единицу измерения" v-model="measureUnit" :start-value="getMeasureUnit" key="product-measure" :disabled="pageModal === 'editAmount'"/>
                    <U-Input v-if="getActiveWarehouse.rule === 'commodity'" class="accounting__input" name="Введите артикул" v-model="article" :start-value="getArticle" key="product-article" :disabled="pageModal === 'editAmount'"/>
                    <U-Input :error="errorAmount" @change="takedAmount = true" class="accounting__input" name="Введите остаток" type="number" v-model="amount" :start-value="getAmount.toString()" key="product-amount" v-if="!accountWeight"/>
                    <template v-if="pageModal === 'add'">
                        <p class="accounting__text">Выберите действие после добавления: </p>
                        <u-radio class="accounting__radio" value="close" name="typeLaterSave" v-model="typeLaterSave" :checked="typeLaterSave === 'close'">Закрыть</u-radio>
                        <u-radio class="accounting__radio" value="addNew" name="typeLaterSave" v-model="typeLaterSave" :checked="typeLaterSave === 'addNew'">Добавить другой товар</u-radio>
                        <u-radio class="accounting__radio accounting__radio--last" value="addNewCurrent" name="typeLaterSave" v-model="typeLaterSave" :checked="typeLaterSave === 'addNewCurrent'">Добавить такой же товар</u-radio>
                    </template>
            ...
            <div :class="['accounting__table', {'accounting__table--commodity': getActiveWarehouse.rule === 'commodity'}]" v-if="getAccounting.length">
                ...
                <template v-for="account in getAccounting">
                    ...
                    <div :class="['accounting__table-item', 'accounting__table-item--amount', {'accounting__table-item--few': !account.weighted && account.amount <= 50 && account.amount > 10, 'accounting__table-item--tiny': !account.weighted && account.amount <= 10  }]">
                        {{account.weighted ? "вес" : account.amount}}
                    </div><div class="accounting__table-item" v-if="getActiveWarehouse.rule === 'commodity'">
                        {{account.article}}
                    </div>
                    <div class="accounting__table-item">
